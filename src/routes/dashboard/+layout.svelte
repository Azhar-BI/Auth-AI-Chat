...
<div class="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100">
				...
				<div class="hidden sm:flex items-center gap-1 text-sm">
					...
					{#if data.role === 'admin'}
						<a
							...
							class="px-3 py-1.5 rounded-lg font-medium transition {$page.url.pathname === ('/dashboard/admin' as string)
								? 'text-black bg-gray-100'
								: 'text-gray-500 hover:text-black hover:bg-gray-50'}"
						>
							Admin
			...
			<div class="absolute top-0 right-0 w-72 h-full bg-white shadow-2xl flex flex-col">
				...
				<nav class="flex flex-col px-6 py-6 gap-1">
					...
					{#if data.role === 'admin'}
						<a
							href="/dashboard/admin"
							class="py-3 px-4 rounded-xl transition font-medium {$page.url.pathname === ('/dashboard/admin' as string)
								? 'text-black bg-gray-100'
								: 'text-gray-700 hover:bg-gray-50 hover:text-black'}"
							onclick={() => mobileMenuOpen = false}
						>
	...
	{#if $page.url.pathname !== '/dashboard/chat'}
		<a
			href="/dashboard/chat"
			class="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-blue-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/30 hover:scale-105 transition-all duration-200 z-50"
